...
Enterprise deployments of AI agents typically grant a static capability envelope at configuration time: a fixed set of tools (file access, code execution, database queries, network calls, external email) that the agent holds for every task in its session, building on foundational paradigms where models learn to invoke external APIs~\citep{schick2023toolformer}. The envelope reflects what the role might ever need, not what any individual task actually requires. We call this gap \emph{context-privilege mismatch}. Preemptively deploying the permissions creates an unnecessary attack surface. A documentation-summarization task with no business reason to make a network call still has \texttt{http\_request} available, and a compromised or manipulated agent can use any tool that is already deployed on the agent environment. Recent work on alignment faking~\citep{greenblatt2024alignment} and frontier-model scheming~\citep{apollo2024scheming} shows that this is not a theoretical concern. Agents capable of reasoning about monitoring systems can adapt their behavior to evade detection, but they cannot use a credential that does not exist.

Existing approaches to agent capability scoping treat the problem as one of input/output filtering or runtime monitoring. Policy-as-Prompt~\citep{tsai2025conseca} automates the translation of design documents into prompt-based classifiers that enforce least privilege at the agent boundary. Output filters and behavioral monitors observe agent actions and intervene when something looks wrong. Although detection mechanisms present a necessary layer of defense, they are reactive countermeasures instead of threat elimination at source. Against agents that reason about monitoring and adapt to evade it, detections have a structural ceiling: the attacker only needs to find one phrasing the monitor misses. Security risk and potential blast radius can be proactively minimized by implementing a dynamic least-privilege methodology. For example, if \texttt{http\_request} permission does not exist in the agent's context for the current task, a misaligned or compromised agent will lose the most straightforward capability to perform HTTP exfiltration. The attack surface is eliminated at the source, reducing the risk earlier in the cyber kill-chain than detective measures.

Effective capability scoping for AI agents requires multiple complementary layers, in the spirit of the Swiss cheese model of defense in depth: a role-based ceiling that constrains an agent to the credentials its function legitimately requires, a task-context classifier that further narrows the permission set to what the current task actually needs, and a policy-derived prohibition layer that eliminates dangerous permission combinations regardless of any individual permission's legitimacy. The third layer specifically targets the \emph{lethal trifecta}~\citep{willison2025trifecta} of private data access, exposure to untrusted content, and external communication, the conditions under which indirect prompt injection~\citep{greshake2023more} enables silent exfiltration. Each layer addresses a unique failure mode: role ceilings cannot distinguish between tasks, task classifiers can be manipulated by injection, and combination prohibitions are deterministic but coarse. Therefore they must be used in combination for effective attack surface minimization.

The architecture supports two deployment modes. In enforcing mode the orchestrator deploys only the predicted credentials and blocks tool calls outside that set. In observe-only mode it deploys the full role-based ceiling and lets the agent execute against it, but still computes the task-context and policy outputs and emits a structured event whenever the agent invokes a tool that the full system would have denied. Observe-only mode mirrors the permissive-mode pattern of access control systems such as SELinux~\citep{smalley2001selinux}, where policy is evaluated and logged before being enforced, and provides a deployment path that does not expose production workflows to undershoot. It also produces a behavioral signal: a record of agent permission requests inconsistent with stated task context, aggregated across tasks and 
deployments. We see this as a contribution to misalignment research that does not depend on the system being run in enforcing mode.
...
\begin{itemize}
  \item \textbf{A proposed three-source capability scoping architecture} that combines role-based ceilings, a task-context classifier, and policy-derived combination prohibitions, framed as a defense-in-depth layer that complements runtime detection. We present the architecture as a design proposal in this paper; empirical evaluation of the full system is left to follow-up work.
  \item \textbf{A synthetic enterprise dataset} of 600 task prompts grounded in a multi-department company policy, labeled across a 15-permission taxonomy that maps directly to deployable credentials or agent guardrails, validated against a 60-record human-reviewed sample at Cohen's $\kappa = 0.967$.
  \item \textbf{A methodological finding on adaptive policy refinement}: synthetic prompt generation acts as a stress test of the underlying access policy, exposing realistic workflow gaps. Co-evolving the dataset and policy reduced ceiling violations from 46 to 3, a 93\% reduction. The dataset, policy, validation results, and metric computation code are released publicly to support replication and future evaluation.
\end{itemize}

\section{Three-Source Architecture}
\subsection{Source 1 - Role-Based Ceiling}
...
\subsection{Source 2 - Task-Context Classifier}
...
the agent cannot influence at runtime. The classifier outputs confidence 
...
\subsection{Source 3 - Policy-Based Combination Prohibitions}
...
a more scalable approach uses Policy-as-Prompt~\citep{tsai2025conseca} to 
...
rejects them, anchors the human to the LLM's prior decisions. Independent 
labeling produces the human's own assessment of what the task requires, 
which the LLM's labels are then compared against. Anchoring bias is 
removed this way. Later the disagreements are surfaced and resolved as 
a separate step. Both pre-review and post-review statistics are used to evaluate the results of the research.

\paragraph{Why Cohen's $\kappa$.} Raw agreement is misleading on this 
dataset: most permissions are denied for most tasks (only 2--4 of the 
15 are typically required), so two raters who default to ``deny'' would 
agree on a large fraction of decisions without doing any real labeling 
work. Cohen's $\kappa$ corrects for this by subtracting the agreement 
that would be expected if both raters made independent random decisions 
with the same marginal frequencies. The remaining quantity is the 
agreement signal that exceeds chance. This is the standard reliability 
metric for two-rater categorical labeling and has been since Cohen 
introduced it in 1960~\citep{cohen1960coefficient}.

We compute $\kappa$ over the in-ceiling decision space which means for 
...
post-review metrics as the headline figures and retain pre-review as 
a stricter floor.
...
post-review figure of 2.0 reflects the LLM's residual rate of high-risk 
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/fig2_pre_post_comparison.pdf}
...
Future work will advance this architecture from a design proposal to an empirically validated system. As an immediate next step, we will train a dedicated task-context classifier on this dataset to establish baseline overshoot and undershoot statistics. We will then evaluate the complete architecture across a phased progression (C0--C3): from an unenforced baseline (C0), to role ceilings only (C1), adding the task classifier (C2), and finally enforcing the policy prohibitions (C3). To further advance AI control against misalignment, we plan to deploy this setup as a custom micro-protocol within the ControlArena~\citep{control_arena} framework. Testing against deceptive model organisms will empirically evaluate the system's capacity to deterministically prevent hidden exfiltration side tasks. Lastly, we will explore practical operational integration via Just-in-Time credentialing and observe-only telemetry for enterprise security platforms.

\section{Conclusion}

To address the context-privilege mismatch and eliminate the attack surface available to deceptive AI agents, we proposed a dynamic capability scoping architecture that shifts the security paradigm from reactive detection to proactive prevention. As a foundational step, we contributed a 600-prompt synthetic enterprise dataset, demonstrating that co-evolving generated tasks with access policies yields a 93\% reduction in architectural ceiling violations. Future work will empirically evaluate this architecture through phased runtime deployments and standardized model organism benchmarks to quantitatively measure its efficacy in neutralizing misalignment in the wild.
