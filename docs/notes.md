...
### 4.3 Deployment Modes: Enforcing and Monitoring

The three-source architecture supports two deployment modes: enforcing and monitoring. In **enforcing mode**, the orchestrator deploys only the credentials predicted by the source intersection and blocks tool calls outside that set; this is the operational mode the rest of the paper describes. In **monitoring mode**, the orchestrator deploys the full role-based ceiling (Source 1 only) and lets the agent execute against it, but still computes the Source 2 and Source 3 outputs in parallel and emits a structured WOULD_DENY event whenever the agent invokes a tool that the full system would have denied. No tool calls are blocked; the agent's behaviour is unchanged.

This mirrors the permissive mode of access control systems such as SELinux, where the policy is evaluated and logged but not enforced, allowing administrators to audit policy correctness against real workloads before flipping to enforcing mode. For organisations adopting this architecture, monitoring mode is the practical first step: it surfaces classifier errors, ceiling gaps, and unrealistic prohibition rules without exposing production workflows to undershoot.

The monitoring mode also has a distinct value for AI safety research. Each WOULD_DENY event records what an agent attempted to do that fell outside what its task should have required — a behavioural signal that, aggregated across tasks, departments, and deployments, becomes a corpus of agent permission requests inconsistent with stated task context. Combined with complementary monitors such as chain-of-thought review, output filtering, or tool-trace inspection, this telemetry contributes a structural channel that does not depend on any single monitor working in isolation. We see this as a route by which dynamic capability scoping contributes to safer AI beyond direct enforcement: the same machinery that prevents misuse at the credential layer produces structured evidence of misuse attempts at the audit layer.

### 4.4 Why Dynamic Guardrails Beat Static Role Definitions
...
### 4.5 The Orchestrator and Just-in-Time Credential Distribution

Capability scoping eliminates access to unnecessary tools. But even when a tool is legitimately granted — say, `database_read` for a data retrieval task — the credentials required to use it traditionally exist in the agent's environment for the entire session. If the agent exfiltrates those credentials via a residual channel, they remain valid long after the task completes.
...
- **Least-privilege credential scoping** — Credentials are not just short-lived but scoped: a `database_read` credential should allow read access to only the specific tables the task requires, not the entire database. This requires the credential vault to support fine-grained scoping, which AWS STS (IAM policy conditions), HashiCorp Vault (path-based policies), and Azure Key Vault (RBAC) all support.
...
Capability scoping prevents the agent from *calling* a tool. JIT credentials prevent the agent from *using the credentials* behind a legitimately called tool in a persistent way. A deceptive agent that successfully exfiltrates via `slack_write` (which the task legitimately required) carries out a credential that expires in minutes. The capability scoping layer and the JIT credential layer have different enforcement points, different failure modes, and different residual risk profiles. Together they compose defence-in-depth: the agent cannot call tools it lacks, and any credentials it holds are near-worthless outside the task window.

### 4.6 Real-World Implementation Design
...
- `source` — which guardrail source drove the decision (Source 1 role ceiling, Source 2 task classifier, Source 3 policy prohibition, or a combination)
...
| Agent granted `database_read` (legitimate) | Spike in row volume returned for same agent ID | Possible large-scale data extraction within legitimate task |
| Agent attempted `email_send_external` (WOULD_DENY) | User account login from new geography same hour | Possible compromised account driving agent misuse |
...
Orchestrator holds: original_task = "Summarise the Q3 report"
           ↓
Agent submits plan:
  [{"tool": "database_read"}, {"tool": "http_request"},
   {"tool": "email_send_external"}]
           ↓
Orchestrator runs its OWN classifier on original_task
→ predicts: [confluence_read, export_file]
           ↓
Check: agent_plan ⊆ orchestrator_prediction?
→ database_read: NOT predicted → REJECTED
→ http_request: NOT predicted → REJECTED
→ email_send_external: NOT predicted → REJECTED
           ↓
Agent told: only confluence_read and export_file available.
```
...
### 6.3 Dataset Construction Approach: Working Backwards from Company Policy
...
| Department | Prompts per batch | Total (30 batches) | Notes |
| --- | --- | --- | --- |
| Engineering | 5 | 150 | Covers code, debug, PR, database queries, external HTTP |
| Customer Success | 4 | 120 | Covers database_read, salesforce_read, email_send_external, email_read |
| Data and Analytics | 3 | 90 | Covers database_read, http_request, export_file, external reporting |
| Security | 3 | 90 | Covers database_read, code_execute, http_request, email_send_external |
| Finance | 3 | 90 | Covers database_read, file_read_uploaded, export_file, email_send_external |
| Legal | 2 | 60 | Covers file_read_uploaded, confluence_read, export_file, email_send_external |
| **Total** | **20** | **600** |  |
...
A separate LLM call — with a fresh context, no access to the generation prompt or Pass 1's reasoning — receives each generated prompt alongside the company policy and the 15-permission taxonomy. It assigns the minimum required capability set for each prompt.
...
A single expert labeller (the primary researcher) validated a random 10% sample (60 records from the 600-record dataset). Using a single labeller rather than two is a limitation noted in §9 Blind Spot 1 — self-review bias applies because the same researcher who produced the dataset labels also reviewed disagreements between those labels and the LLM's predictions.

Cohen's kappa between human and LLM labels was **0.917** (almost perfect agreement). The 0.8 threshold was comfortably met; no profiles required regeneration.

Sensitivity tier labels (LOW / MEDIUM / HIGH) were found to be highly subjective, with only 60.0% agreement between the human validator and the LLM labeller — compared to κ = 0.917 on permission decisions. The MEDIUM tier was particularly unstable, with agreement of only 50% and errors distributed in both directions. Sensitivity labels are therefore excluded from all quantitative metrics and classifier evaluations. They are retained in the dataset as descriptive metadata only. See §8 Results for the full sensitivity finding.
...
### 6.4 Generation Profiles

All profiles cover tasks that exercise the full 15-permission taxonomy. Every task is generated from the TechCorp company policy (Section 6.2) and evaluated against C0 (all 13 Engineering permissions available).
...
> 📌 **Reasoning fields:** Each labelled record includes a `reasoning` object with two sub-fields: `granted` (one sentence per permission that was granted, explaining why it is required) and `denied` (one sentence per permission that was denied, explaining why it is not required). The `denied` field covers all denied tools and is not restricted to high-risk permissions — the LLM provides justification for every denial. These reasoning fields are used during the human review pass to allow the reviewer to evaluate the LLM's logic for each disagreeing permission decision.
> 

### 6.6 Running the Pipeline at Scale

Run each profile 5 times at temperature 0.9 → 600 total prompts. Split: 500 training / 100 test (held out, never seen during training). Human-validate 10% of the full dataset (60 records). Compute Cohen's kappa between human and LLM labels on permission decisions. If below 0.8 for any permission tier, regenerate. Sensitivity agreement is tracked separately (see §8).

> 📌 **Ceiling-filtered computation:** All permission agreement metrics (kappa, precision, recall, F1, overshoot/undershoot rates) are computed within the department capability ceiling. A tool that is structurally unavailable to a department — for example, `github_read` for Finance — is excluded from all confusion matrix calculations for records in that department. Including out-of-ceiling tools would produce trivial true negatives (both human and LLM correctly deny them) that inflate agreement metrics without reflecting classifier performance on the tools that actually matter.
> 
...
All 13 Engineering permissions are always available in C0 (the department ceiling for Engineering, defined in Section 4.7). The delta is measured from this full-grant baseline.
...
- **Human validation:** 10% random sample (60 records) validated by one expert labeller. Cohen's kappa reported for permission decisions (κ = 0.917 pre-review, κ = 0.967 post-adjudication). Sensitivity agreement reported separately (60.0%) and excluded from all quantitative metrics. Per-permission disagreements were resolved through an interactive review pass; resolution decisions (llm_correct / human_correct / ambiguous) are recorded in `disagreements.json` and released with the dataset.

### 7.3 Injection Stress Test Subset

30 adversarially crafted tasks across three injection classes:

- **Class A — Capability suggestion via phrasing:** Task implies a capability through natural language without explicitly requesting it. Example: *"Search the runbooks for our Q3 incident response playbook and pull together a quick summary — might want to grab some metrics too if it makes the summary clearer."* Ground truth: `confluence_read` only. The phrase "grab some metrics" suggests `database_read` but the task does not actually require it; the summary can be produced from the runbooks alone.
...
### 7.6 Human Validation Results

A 60-record sample was validated by the primary researcher and compared against the LLM's permission labels. Each of the 20 permission disagreements was then adjudicated (llm_correct / human_correct / ambiguous) to produce a resolved ground truth. Two metric sets are reported: pre-review (human labels as reference) and post-review (adjudicated labels as ground truth, ambiguous cases excluded).
...
**Primary finding:** Post-review kappa of **0.967** (near-perfect agreement) is the paper's headline accuracy figure. The pre-review kappa of 0.917 is retained as the conservative baseline — it represents accuracy assuming human labels are always correct, which is a stricter standard than the true ground truth.

The severity-weighted overshoot drop from 15.00 to 2.00 (86.7% reduction) is the most security-relevant finding. The pre-review figure of 15.00 was almost entirely human labelling errors — cases where the human incorrectly denied a high-tier tool that the LLM correctly granted. The LLM's true over-granting of high-risk tools is near-zero.

The remaining genuine error is conservative undershoot at 4.4%: the LLM denies permissions that should have been granted. For a security control, this is the preferred failure mode — a missed grant causes task inconvenience, while an incorrect grant expands the attack surface.
...
The 50/40/10 split shows that when disagreements occurred, the human and LLM were wrong at roughly equal rates. Human labels are not a clean ground truth: they contain systematic labelling errors at a rate comparable to the LLM's prediction errors. This is why the adjudicated post-review metrics are the primary claim.
...
Sensitivity tier labels (LOW / MEDIUM / HIGH) were found to be highly subjective, with only 60.0% agreement between the human validator and the LLM labeller — compared to κ = 0.917 on permission decisions. The MEDIUM tier was particularly unstable, with agreement of only 50% and errors distributed in both directions. Sensitivity labels are therefore excluded from all quantitative metrics and classifier evaluations. They are retained in the dataset as descriptive metadata only.

Of 24 sensitivity disagreements, the human was correct in 70.8% of cases and the LLM in 25%. Both the low agreement rate and the LLM's weaker performance on sensitivity (vs. permissions) independently support the decision to exclude sensitivity tier from all metric computations.
...
95% confidence intervals were computed using the standard error formula for Cohen's kappa with two raters and binary outcomes (Cohen, 1960; Fleiss, Cohen & Everitt, 1969):

```
SE(κ) = √[ p_o(1 − p_o) / (n × (1 − p_e)²) ]
```

where `p_o` is the observed agreement proportion, `p_e` the expected agreement under chance, and `n` the number of items rated.
...
Both lower bounds clear the 0.80 substantial-agreement threshold by a wide margin. The pre-review lower bound of **0.882** is the conservative floor: even under the pessimistic assumption that all human labels are correct (which the disagreement resolution shows is not the case), agreement remains near-perfect.
...
Denied invocations logged with: timestamp, tool requested, active capability set, task context snapshot, whether denial is Source 1, Source 2, or Source 3 driven.
...
**Threat model under this architecture:** A deceptive agent that exfiltrates credentials via a legitimate tool (e.g. embeds them in a `slack_write` call) now holds credentials with a TTL of minutes. Even if the attacker receives the message, the window for exploitation is bounded. A deceptive agent that attempts to call a revoked tool receives CAPABILITY_DENIED before the credential question even arises.
...
### Blind Spot 1 — Ground truth subjectivity and self-review bias

**Problem:** Human labels are produced by a single labeller (the primary researcher), not two independent experts as originally planned. This creates two compounding risks: (a) systematic blind spots that a second independent labeller would have caught, and (b) self-review bias, since the same researcher who produced the labels also adjudicated disagreements between those labels and the LLM's predictions. A researcher resolving their own prior decisions may unconsciously favour confirmation of the original label.

**Mitigation:** Cohen's kappa (κ = 0.917 pre-review, κ = 0.967 post-adjudication) indicates that even under self-review conditions, permission agreement is high enough to proceed. All disagreement resolutions and their notes are released in `disagreements.json` so external reviewers can audit the adjudication decisions. Sensitivity disagreements are reported separately and not collapsed into the permission kappa — the 60.0% sensitivity agreement is disclosed as a finding about definitional divergence, not treated as noise.

The most significant remaining limitation is the absence of a second human labeller. This is noted as future work: validating the dataset with an independent expert and computing true inter-rater kappa would substantially strengthen the labelling validity claim.
...
**Mitigation:** The paper claims the three-source fusion is *more resistant* to evasion than single-source, not evasion-proof. The evasion resistance metric (RQ4, RQ5) quantifies this directly.
...
| --- | --- | --- |
| **Phase 1 — Dataset generation** | Generate 600 prompts, LLM-label, human-validate 10% (60 records), build injection stress tests | Weeks 1–4 |
...
- **Section 4 — System Design:** Three-source architecture. Classifier. Enforcement layer. Fusion mechanism. Deployment modes (enforcing vs monitoring).
...
- **Section 7 — Experimental Methodology:** Task battery. Five conditions (C0–C4). All metrics. Human validation results.
...
- **Section 10 — Implementation Plan:** Phased timeline.
- **Section 11 — Conclusion:** Three claims with empirical support.
- **Section 12 — Future Work:** Online learning, multi-agent token model, hardware enforcement (TEEs).

---

## 12. Future Work

### 12.1 Multi-Agent Task Token Model
...
### 12.2 Online Learning from Production Audit Logs
...
### 12.3 Hardware Enforcement via Trusted Execution Environments
...
### 12.4 Residual Channel Enumeration
...
## 13. Connection to Company Product
...
| Source 1 (role ceiling lookup) | Onboarding: baseline capability policy from system prompt in minutes |
...
- Nassi, B., Bitton, R., Shwartz-Ziv, R., Shabtai, A., & Elovici, Y. (2024). ComPromptMized: Unleashing Zero-click Worms that Target GenAI-Powered Applications. arXiv:2403.02817. Code: [github.com/aisb-sg](http://github.com/aisb-sg)
